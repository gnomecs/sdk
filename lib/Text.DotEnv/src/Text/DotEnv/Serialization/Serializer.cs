...
internal static class Serializer
{
    ...
    public static void Serialize(TextWriter writer, object? obj, [Dam(Dat.PublicProperties)] Type type, DotEnvSerializerOptions? options = null)
    {
        ...
        {
            if (typeof(IReadOnlyDictionary<string, string?>).IsAssignableFrom(type))
            {
                SerializeDictionary((IReadOnlyDictionary<string, string?>)obj, writer, options);
                return;
            }

            if (typeof(IReadOnlyDictionary<string, object?>).IsAssignableFrom(type))
            {
                SerializeDictionary((IReadOnlyDictionary<string, object?>)obj, writer, options);
                return;
            }
        ...
        {
            if (prop.GetCustomAttribute<DotEnvIgnoreAttribute>() is not null)
                continue;
            ...
            var name = prop.Name;
            var serializedAttr = prop.GetCustomAttribute<DotEnvNameAttribute>();
        ...
        DotEnvSerializerOptions? options = null)
    {
        ...
        foreach (var item in dictionary)
        {
            ...
            if (item.Value is string v && v.IsNullOrWhiteSpace())
            {
                writer.Write('\"');
                writer.Write('\"');
            }
        ...
        [Dam(Dat.PublicConstructors | Dat.PublicProperties)] Type type)
    {
        ...
        foreach (var prop in props)
        {
            ...
            if (prop.GetCustomAttribute<DotEnvIgnoreAttribute>() != null)
            {
                continue;
            }
            ...
            var serializationAttr = prop.GetCustomAttribute<DotEnvNameAttribute>();
    ...
    private static void SetValue(PropertyInfo prop, object obj, string? value)
    {
        ...
        switch (code)
        {
            ...
            case TypeCode.DateTime:
                prop.SetValue(obj, DateTime.Parse(value, CultureInfo.InvariantCulture, DateTimeStyles.RoundtripKind));
                break;
            ...
            default:
                ...
                {
                    prop.SetValue(obj, DateTimeOffset.Parse(value, CultureInfo.InvariantCulture, DateTimeStyles.RoundtripKind));
                    break;
                }
