...
public class AppPaths : IAppPaths
{
    ...
    public AppPaths(ApplicationPathsOptions? options = null, IAppEnv? environment = null)
    {
        ...
        if (options.AppDirectoryName.IsNullOrWhiteSpace())
            options.AppDirectoryName = environment?.Name ?? AppEnv.GetAssemblyName();
    ...
    public static IAppPaths Current { get; set; } = new UnknownAppPaths();
