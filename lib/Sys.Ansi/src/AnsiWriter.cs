...
{
    private readonly ICommandMasker? masker;

    private AnsiLogLevel level = AnsiLogLevel.Information;

    public AnsiWriter(ICommandMasker? masker = null)
        => this.masker = masker;
    ...
    public virtual void Command(string command, string[] args)
    {
        if (args.Length > 0)
        {
            string masked = string.Join(" ", args);
            if (this.masker is not null)
                masked = this.masker.Mask(masked);
